...
const FileInput: React.FC<FileInputProps> = ({ id, label, onFileChange, required = false, isSelected = false, note, onSelectGallery, value }) => {
    ...
    <div>
      ...
      <div className={`mt-1 flex flex-col justify-center items-center px-6 pt-5 pb-6 ${borderStyle} rounded-md transition-all`}>
        ...
        <div className="space-y-1 text-center">
          {value || preview ? (
            <div>
              <img src={value ? `data:${value.mimeType};base64,${value.data}` : preview!} alt="Preview" className="mx-auto h-24 w-24 object-contain rounded-md" />
              <p className="text-xs text-gray-500 mt-2 truncate max-w-xs">{fileName || 'Selected from gallery'}</p>
            ...
            <>
              ...
              <div className="flex text-sm text-gray-600 space-x-1">
                ...
                {onSelectGallery && (
                  <>
                    <span>/</span>
                    <button type="button" onClick={onSelectGallery} className="font-medium text-[#4285F4] hover:text-blue-500">
                      Select File
                    </button>
                  </>
                )}
