...
styleVariations = [
    ...
    "**Bold Typographic Focus:** Create a dynamic, modern layout where typography is a key artistic element. ASSET 1 must be placed freely and integrated naturally into the overall scene without being forced into a constricted split-screen box or cropped window. You may use vibrant bold color blocks *behind the text* to make the typography pop, but these graphic blocks must NOT trap, crop, or constrain the product. ASSET 1 MUST remain perfectly intact, undistorted, and uncropped. The result should feel energetic and deliberate.",
...
- **USE ONLY THE STYLE:** Extract colors, typography, and overall mood.
- **UNIFIED BACKGROUND (CRITICAL):** The final ad must have a single, cohesive background. Do NOT create a disjointed 'split' effect where half the image is a realistic 3D room and the other half is a flat template graphic. Do NOT render ASSET 2 as a physical poster hanging on a wall. The entire canvas must be a unified scene or a unified graphic layout.
...
- **PRODUCT INTEGRITY (ABSOLUTE PRIORITY):** ASSET 1 is a mathematically immutable digital asset. You MUST treat it as a strict overlay.
  1. **Zero Redrawing:** It must remain pixel-perfect identical to the uploaded image. NEVER redraw, sketch, or synthetically recreate the product.
  2. **No Lighting or Color Contamination:** Do NOT adjust the product's internal lighting, contrast, or color to match the newly generated background. Do not let environmental light (e.g., sunset orange) tint the product's original color.
  3. **No Masking/Cropping Distortion:** Do NOT crop, stretch, squash, or force the product into unnatural geometric frames if it cuts off or warps the product.
  4. **Background Shadows Only:** You may generate shadows cast *from* the product onto the background floor/wall to ground it, but the pixels *on the product body* must remain 100% untouched.
- **REALISTIC SCALE & PROPORTION (CRITICAL):** Place the product so its physical scale is completely logical and realistic relative to its surrounding environment (e.g., furniture, rooms, people, TV). For example, a floor-standing air conditioner must be realistically scaled to stand significantly taller than a TV stand. Do not make the product unnaturally miniature or giant just to arbitrarily fit the canvas.

### 3. Logo Integration (ASSET 3)
- **Harmonious Blending & Background:** Adapt or remove the exact background color of ASSET 3 so that the logo blends naturally and seamlessly into the newly generated ad background. Ensure it maintains pristine visibility and high contrast.
- **Smart Scaling:** Intelligently adjust the scale of the logo so it fits beautifully within the current aspect ratio. It should occupy an appropriate proportion (roughly 5-10% of the area) so it is neither overpowering the product nor too small to read.
- **Core Identity:** Do not redraw or distort the core shape, typography, and symbol of the brand logo itself.
...
- **COMPLETE RENDERING & NO DUPLICATION (CRITICAL):** You MUST render EVERY piece of text provided in the "Ad Copy" section (Headline, Description, CTA) exactly ONCE. Do NOT omit any text, and **DO NOT duplicate, echo, or repeat** any text anywhere in the image (e.g., never write the headline twice).
- **VISUAL HIERARCHY & STYLING (CRITICAL):** Apply distinct styles to each element:
  - **Headline:** Must be the largest, boldest, and most dominant text element.
  - **Description:** Must be smaller than the Headline, highly legible, and act as supporting text.
  - **CTA:** Must be styled distinctly and prominently as an actionable element (e.g., framed as a button or badge), drawing inspiration from button styles in ASSET 2.
- Ensure all text is perfectly legible with high contrast against its background.
- **ZERO HALLUCINATION (CRITICAL):** Use ONLY the exact ad copy provided above. You MUST NOT add any extra meaningless words, gibberish text, fake buttons, or decorative alphabet letters.
- **IGNORE TEMPLATE TEXT (CRITICAL):** Ignore any existing text, sub-copy, or words shown in the style guide (ASSET 2). Do not copy, mimic, or invent filler text. If a word is not explicitly listed in the "Ad Copy" section, DO NOT write it!
'''
    else:
        prompt += '''
- **ABSOLUTELY NO TEXT (CRITICAL):** The user did not provide ad copy. You MUST NOT generate any words, alphabetical letters, typography, watermarks, or fake text placeholders anywhere in the image.
- **Visual Focus & Negative Space:** Design the ad using only the product, the logo, and the background environment. Leave organic, clean empty space (negative space) so the final composition looks complete and balanced even without any text.
'''
